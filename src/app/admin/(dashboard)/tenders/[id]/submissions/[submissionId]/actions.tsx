...
export function SubmissionDetailActions({ tenderId, submissionId }: SubmissionDetailActionsProps) {
  const handlePrint = useCallback(() => {
    window.print();
  }, []);

  const handleDownload = useCallback(() => {
    window.open(`/api/submissions/${submissionId}/pdf`, '_blank');
  }, [submissionId]);
    ...
    <div className="submission-actions-bar sticky top-0 z-50 -mx-4 -mt-4 mb-6 flex items-center justify-between gap-3 border-b border-stone-700 bg-stone-900/95 px-4 py-3 backdrop-blur sm:-mx-6 sm:-mt-6 sm:px-6 lg:-mt-6 print:hidden">
      ...
      <div className="flex items-center gap-2">
        ...
        <button
          type="button"
          onClick={handlePrint}
          className="inline-flex items-center gap-2 rounded-md bg-stone-700 px-3 py-2.5 text-sm font-medium text-stone-300 hover:bg-stone-600 hover:text-stone-100 transition-colors"
        >
          <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
